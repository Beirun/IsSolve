...
const SubmitReport = () => {
  ...
  const handleIssueTypeOnChange = (event,value) =>{
    ...
    setUserReport((userReport)=>({...userReport,'issueType':value ==='OTHER'? typedIssueType: value }))
    setErrors((errors) => ({ ...errors, issueType: "", otherissueType: "" }));

  }
  const handleOtherOnChange = (e) =>{
    setTypedIssueType(e.target.value)
    setUserReport((userReport) => ({...userReport,'issueType':e.target.value}))
    setErrors((errors) => ({ ...errors, otherissueType: "" }));
}
const handleDescriptionOnChange = (e) =>{
    setUserReport((userReport) => ({...userReport,'description':e.target.value}))
  ...
  const handleSubmitReport = async() =>{
    ...
    if(newErrors.otherissueType || newErrors.issueType || newErrors.description || newErrors.proofImage || newErrors.location){
      return
    }

    const report = await createReport(userReport)
    if(report){
  ...
  console.log('errors',errors.otherissueType)
      ...
      >
        ...
        >
          ...
          <Box
            ...
            <Box>
              <Button
                ...
                sx={{
                  opacity: userReport.proofImage? '100%' : errors.proofImage? '100%' : "23%",
                  width: "21.224vw",
                  height: "24.626vh",
                  overflow:'hidden',
          ...
          backgroundPosition: "center",
                ...
                (
                    <Box sx={{display:'flex', justifyContent:'center',alignItems:'center'}}><IconFileUpload size={100} stroke={1}/>
                    <Box sx={{height:'10.884vh', display:'flex',justifyContent:'space-between',flexDirection:'column'}}>
                        <Typography sx={{fontFamily:'Roboto', fontWeight:'400'}} >DRAG AND DROP</Typography>
                        <Box sx={{display:'flex',flexDirection:'row', justifyContent:'space-between', alignItems:'center', }}>
                            <Box sx={{width:'35%', height:'1px', backgroundColor:'white'}}/> 
                            <Typography sx={{fontFamily:'Roboto', fontWeight:'400'}}>OR</Typography>
                            <Box sx={{width:'35%', height:'1px', backgroundColor:'white'}}/> 
                        </Box>
                        <Box sx={{display:'flex', justifyContent:'center'}}><Typography sx={{fontFamily:'Roboto', fontWeight:'400'}}>UPLOAD A FILE</Typography></Box>
                    </Box></Box>
                )}
                <VisuallyHiddenInput
              type="file"
            ...
            >
              <Autocomplete
                ...
                sx={{ width: "16.081vw" }}
                renderInput={(params) => (
                  <TextField error={errors.issueType !== "" } helperText={errors.issueType}  {...params} color="inherit" label="ISSUE TYPE" />
                )}
                onChange={handleIssueTypeOnChange}

              />
              <Box
                sx={{ height: "10.483vh", width: "16.081vw" }}
              >
              <TextField
        ...
        <Box
          sx={{
            width: "39.583vw",
            height: "69.252vh",
            border: errors.location ? "3px solid #F44336" : "3px solid transparent",
          }}
        >
          <MapContainer
